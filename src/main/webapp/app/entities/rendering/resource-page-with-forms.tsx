...
export const smForm3 = (smForm: SmFormResourceContent) => {
...
export const RenderResourcePageWithForms = () => {
  ...
  const startProcessMarkup = (resource: ComponentResourceContent): { html: string; options: any } => {
    // const fo = smForm3(childForm);
    // const { register, handleSubmit, reset, unregister } = useForm({ defaultValues: {} });
    return processMarkup(resource, null, null, {}, null);
  };
  ...
  const processMarkup = (
    ...
    if (content.componentType === 'SmMarkup') {
      ...
      return processMakupElement(markup, form, content.inputParameters, html, options, key);
    } else if (content.componentType === 'form') {
      const form: SmFormResourceContent = content as SmFormResourceContent;
      return processMakupElement(form.params.formContent as SmMarkupResourceContent, form, content.inputParameters, html, options, key);
    }
  };

  const processMakupElement = (
    markup: SmMarkupResourceContent,
    form: any,
    ...
    keys.forEach(key => {
      ...
      console.log('rootprocessMarkup=====', key, child, inputParameters);
      if (child.componentType === 'SmMarkup') {
        ...
        const { html: html1, options: options1 } = processMarkup(childMarkup, form, newHtml, options, key);
        newHtml = html1;
      } else if (child.componentType === 'form') {
        const childForm: SmFormResourceContent = child as SmFormResourceContent;
        // console.log('rootprocessMarkupchild=====', childMarkup);
        // const fo = smForm3(childForm);
        const { html: html1, options: options1 } = processMarkup(childForm, childForm.params.form, newHtml, options, key);
        newHtml = html1;
      } else {
